...
async def create_text_channel(guild_id: int, channel_name: str, topic: Optional[str] = None) -> str:
    """
    Creates a new text channel in the server.
    
    Args:
        guild_id: The ID of the guild (server) where the channel should be created.
...
async def create_voice_channel(guild_id: int, channel_name: str) -> str:
    """
...
async def delete_channel(guild_id: int, channel_id: int, reason: Optional[str] = None) -> str:
    """
...
async def make_channel_private(guild_id: int, channel_id: int, role_id: int) -> str:
    """
    ...
    Args:
        ...
        role_id: The ID of the role that is allowed to view the channel.
    """
    if not _bot:
        return "Error: Bot not initialized."
    guild = _bot.get_guild(guild_id)
    ...
    if not channel or not role:
        return "Error: Channel or Role not found."
...
async def create_role(guild_id: int, role_name: str, color_hex: Optional[str] = None) -> str:
    """
...
async def delete_role(guild_id: int, role_id: int) -> str:
    """
    ...
    role = guild.get_role(role_id)
    if not role:
        return f"Error: Role with ID {role_id} not found."
...
async def assign_role(guild_id: int, member_id: int, role_id: int) -> str:
    """
...
async def remove_role(guild_id: int, member_id: int, role_id: int) -> str:
    """
...
async def kick_member(guild_id: int, member_id: int, reason: Optional[str] = None) -> str:
    """
    Kicks a member from the server.
    
    Args:
        guild_id: The ID of the guild.
        member_id: The ID of the member to kick.
...
async def ban_member(guild_id: int, member_id: int, reason: Optional[str] = None) -> str:
    """
    Bans a member from the server.
    
    Args:
        guild_id: The ID of the guild.
        member_id: The ID of the member to ban.
...
async def timeout_member(guild_id: int, member_id: int, duration_minutes: int, reason: Optional[str] = None) -> str:
    """
    Temporarily times out (mutes) a member so they cannot send messages or talk in voice channels.
    
    Args:
        guild_id: The ID of the guild.
        member_id: The ID of the member to timeout.
...
async def untimeout_member(guild_id: int, member_id: int) -> str:
    """
...
async def send_message_in_channel(guild_id: int, channel_id: int, content: str) -> str:
    """
...
async def search_members(guild_id: int, query: str) -> str:
    """
